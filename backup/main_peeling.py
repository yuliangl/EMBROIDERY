...
sigma = np.deg2rad(0)  # rotation angle of one time
band = 0.05
yos = 0.005
z_axis = [0, -1, 3.732]
...
normal, bdy_sizes = PointCloudProcess.boundaryNormal(matrix_c2b_rgb)
...
"""# hold apple and take it to fixed place
center_up = [0.65, 0, 0.32, 0, 0, 0]
...
ARM_L.move_rmrc([0.55, -0.47, 0.22+band, -75*toRad, 0, np.pi/2])
ARM_L.move_rmrc([0.55, -0.47, 0.12+band, -75*toRad, 0, np.pi])
...
for k in range(30):
	...
	while (i < p_sizes - 2):
		y_axis = [0, 3.732, 1]
		beta = normal_angle(pl_normal[i], z_axis)
		if (np.cos(beta) < 0):
			i += 1
			continue
		for j in range(i + 1, p_sizes - 2):
			beta = normal_angle(pl_normal[j], z_axis)
			if (np.cos(beta) < 0):
				i = j
				break
			alpha = normal_angle(pl_normal[i], pl_normal[j])
			if (alpha >= theta):
				initNode.append(i)
				node.append(j)
				i = j
	...
	node.insert(0, initNode[0])

	if len(node) == 4:
		complement = (node[3] + p_sizes) / 2 + 2
		node.append(complement)
	elif len(node) == 3:
		complement = (node[2] + p_sizes) / 2 + 2
	...
	for i in range(len(node) - 1):
		...
		if i == 0:
			# ARM_R.simulation_flag = False
			ARM_R.move_rmrc(force_ori)
			print "reach first point of ", k, "times"
			behind[1] -= yos   #0.004 +
			ARM_R.otc_setFOffset()
			ARM_R.mode_fMove()
			# touch_p[1] = -0.60
			ARM_R.move_fMoveLim(force_ori, 1, target_force=1.0, ctrl_axis=[0, 1, 0, 0, 0, 0], v_max=0.001)
			ARM_R.otc_setFOffset()
			ARM_R.otc_setForceCoordinate(behind)
			ARM_R.move_fMoveTo(behind, target_force=1.0, ctrl_axis=[0, 1, 0, 0, 0, 0], v_max=0.003)
		else:
			# ARM_R.simulation_flag = False
			# ARM_R.getState()
			...
			if (i == len(node) - 2):
				behind[1] -= yos + 0.001
				ARM_R.move_fMoveTo(behind, target_force=5.0, ctrl_axis=[0, 1, 0, 0, 0, 0], v_max=0.002)
			ARM_R.otc_setForceCoordinate(behind)
			ARM_R.move_fMoveTo(behind, target_force=5.0, ctrl_axis=[0, 1, 0, 0, 0, 0], v_max=0.003)
		ARM_R.mode_rmrc()
